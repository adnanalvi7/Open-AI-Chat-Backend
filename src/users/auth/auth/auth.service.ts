...
import { CreateUserDto, ForgotPasswordDto, loginDTO, ResetPasswordDto, VerifyOtpDto } from 'src/users/users.interface';
...
export class AuthService {
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto): Promise<{message:string}> {
    ...
    return {message:`OTP for ${forgotPasswordDto.email} has sent `}
  }


  async verifyOTP(verifyOtpDto: VerifyOtpDto): Promise<boolean> {
    const storedOtp = await this.redisClient.getKey(`otp:${verifyOtpDto.email}`);
    await this.redisClient.setKey(`otp_verified:${verifyOtpDto.email}`, 'true', 600);
    return storedOtp === verifyOtpDto.otp;
  }

  async resetPassword(resetPasswordDto: ResetPasswordDto): Promise<void> {
    const isOtpVerified = await this.redisClient.getKey(`otp_verified:${resetPasswordDto.email}`);
    ...
    await this.redisClient.deleteKeys([`otp:${resetPasswordDto.email}`,`otp_verified:${resetPasswordDto.email}`]);
